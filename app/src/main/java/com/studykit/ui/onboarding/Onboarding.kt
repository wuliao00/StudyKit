...
private val onboardingPages = listOf(
    // 第一页是**引导**而不是介绍：首启是空库（2.4.5 起分发包不再播种演示数据），
    // 新用户面对的是四个空白页签，最需要知道的是"我现在该点哪里"，
    // 而不是"这个应用有哪些功能"。
    TutorialPage(
        "从这里开始（三步就有内容）",
        listOf(
            "① 先把词装进来：「学习 → 录入」可以手输一条、批量粘贴、截图取词，" +
                "或者从「在线词库」下载一份现成词表。",
            "② 再建一个习惯：「习惯 → 添加」，每天点一次那个圆钮就算打卡。",
            "③ 想逼自己一把就签一份契约：「习惯 → 自我契约」，到期按真实打卡数对账。",
            "三件事任何顺序都行，先做一件也行 —— 数据只在你手机上，随时可以从设置里导出。",
        ),
    ),
    TutorialPage(
        "学习：把词装进来，再翻着复习",
