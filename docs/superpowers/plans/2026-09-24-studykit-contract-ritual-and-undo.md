...
### R5–R8（合并前整枝评审追加的裁决；原有 R1–R4 的正文一字不动）

编号沿用控制器合并前那份评审清单，对应关系是 **R5↔I5、R6↔I6+I7、R7↔I2+I3、R8↔I4**；
代码注释里出现的"裁决 R5/R6/R7/R8"指的就是下面四条。

#### R5 本计划 R1 的**前提**是错的（结论不受影响）

R1 原文写"对账是惰性的，只有进契约页时才跑"。不成立：`ContractsViewModel.init` 收集的是源头
`observeAll()`、跑在 `viewModelScope` 里，而这个 ViewModel 在 `ui/nav/AppNav.kt` 的根 composable
就 `viewModel()` 出来了（`MainActivity.setContent` 一进来必定走到），所以**结算跑在整个进程寿命里**。
修正直接写在上面 R1 的日期块里（那里保留了被推翻的原话），本条只负责把编号补齐 ——
代码注释引用 R 编号时不该撞上一个没有解释的空洞。
连带后果：不在契约页时结算出来的那几张会攒在队列里，这是 R8 那行计数存在的理由。
