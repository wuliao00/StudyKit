...
> **§5 落地形态回写（2026-09-21，与代码不一致处以此为准）**
>
> - **§5.1 分列**：实际是四级优先序（制表符 → 连续两个以上空格 → 半/全角逗号 → 兜底第一个单空格），
>   且每级都要切出 ≥2 个非空段才算命中；1–3 级 `limit = 3`，因为释义本身常含逗号。
>   行号 1 基、空行也占号（坏行要能指回用户刚贴的那一行）。
>   已知固有歧义：`ice cream 冰淇淋`（纯单空格、无标点）会被兜底切成词头 `ice`；带逗号或双空格即正确。
> - **§5.1 题目**：答案支持「字母」与「选项原文」两种写法；判定顺序是先试最后一列，
>   再试倒数第二列（此时最后一列当解析列）。**多选题不支持**（模型只有一个答案位），进「待修正」。
> - **§5.1 摘录**：解析器（首尾竖线定界，摘录内的竖线不切碎）与单测都在，
>   但**批量导摘录未实现** —— `excerpts` 要求已存在的 `book_id`（外键级联）且没有承载「感想」的列。
>   故 `ImportKind` 只开放 WORD / QUESTION，摆一个只会提示"不支持"的按钮比不摆更糟。
> - **§5.2 词库**：词表 zip 内只有一个 `<id>.json`，**内容是 NDJSON**（整文件不是合法 JSON），
>   解析按行、坏一行只坏一行；`offlinedata` 已是绝对地址，不需拼 GitHub 路径。
>   该主机 **不提供 HTTPS**（实测无响应），所以明文放行只针对 `ydschool-online.nos.netease.com` 这一个域，
>   没有用全局 `usesCleartextTraffic`。目录快照随版本打包（68KB / 81 本）。
>   导入的单词挂 `source_list_id`，因此支持**整本撤销**（只删这本带进来的词）。
> - **§5.3 分享**：清单已注册 `SEND` / `SEND_MULTIPLE`（`image/*`）+ `singleTop`，
>   分享进来的图先拷进相机临时目录再走既有通道（下游零分支）。
>   ~~分享即自动预识别已在真机验证可用~~ → **自动预识别的代码与路由已实现，但真机未验证**：
>   `am start` 造的四种 intent 都没能产出临时文件，无法区分"harness 造不出真实授权"与"功能确有 bug"。
>   已把静默失败改成有提示，待一次真实分享定论。
> - **§5.4 OCR**：bundled 中文模型、全离线、无需 GMS，已在真机跑通（中文题图 → 标题/备注正确回填）。
>   错题录入页是手动按钮 + 回填可编辑、**不自动保存**。
>   详情页「长按识别」**未实现**：详情页只有改学科与删除，没有编辑正文的入口（另立任务）。
>   体积代价：原生库四 ABI 共 41MB，本仓直接发 APK 故只留 ARM 两个 ABI（56.9MB → 33.7MB）。
> - **§5.5 结果卡**：除三个计数外，「重复跳过」与「待修正」都能展开看**逐条明细** ——
>   判重是全局的，只给数字会让用户以为数据被吞。
